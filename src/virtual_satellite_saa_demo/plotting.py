...
def ground_track_figure(
    ...
    index: int,
    *,
    show_radiation_areas: bool = True,
    ...
    for path in _land_paths():
        ax.add_patch(
            PathPatch(
                path, facecolor="#26394b", edgecolor="#718497", linewidth=0.55, zorder=0
            )
        )
    if show_radiation_areas:
        ...
        ax.contourf(
            ...
            lat,
            intensity,
            levels=np.linspace(0.05, 1.25, 20),
            cmap="magma",
            alpha=0.45,
            zorder=1,
        )
        ...
        for latitude in (-76, 76):
            ax.text(
                ...
                color="#fcbe66",
                fontsize=9,
                bbox={"facecolor": "#0e1726", "alpha": 0.7, "edgecolor": "none"},
                zorder=2,
            )
    end = 0 if background_only else index + 1
    x, y = track.longitude_deg[:end], track.latitude_deg[:end]
    # Insert breaks rather than drawing lines across the map at the dateline.
    breaks = np.flatnonzero(np.abs(np.diff(x)) > 180) + 1
    ax.plot(
        np.insert(x, breaks, np.nan),
        np.insert(y, breaks, np.nan),
        color="#67dce5",
        lw=1.0,
        alpha=0.35,
        label="Ground track",
    ...
    counts = radiation.counts[:end]
    events = counts > 0
    ax.scatter(
        x[events],
        y[events],
        s=12 + 9 * np.sqrt(counts[events]),
        c="#ffb45b",
        alpha=0.8,
        label="Bit upsets",
        zorder=3,
...
def memory_figure(memory: np.ndarray) -> Figure:
    ...
    ax.set(
        xlabel="Bit column", ylabel="Row", title="4,096 bits · blue = 0 · yellow = 1"
    )
    return fig
