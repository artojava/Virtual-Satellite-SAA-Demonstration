...
from virtual_satellite_saa_demo.orbit import SIDEREAL_DAY_S, OrbitConfig, simulate_orbit
...
def test_circular_orbit_and_earth_rotation():
    ...
    assert track.longitude_deg[-1] == pytest.approx(
        -90 - 360 * config.period_seconds / SIDEREAL_DAY_S
    )
...
def test_equatorial_orbit_and_speed():
    ...
    assert (
        OrbitConfig(speed_multiplier=2).period_seconds
        == OrbitConfig().period_seconds / 2
    )
